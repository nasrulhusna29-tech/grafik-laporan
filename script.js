...
async function initLoad() {
    ...
    try {
        for (let b of listBulan) {
            document.getElementById('loaderText').innerText = `Memproses Data ${b}...`;
        ...
        alert("Gagal memuat data. Periksa koneksi atau Spreadsheet ID.");
    } finally {
...
function applyFilters() {
    ...
    poliList.forEach(nama => {
        ...
        if(sT > 0) { tableData.push({ nama, l: sL, p: sP, t: sT }); gL += sL; gP += sP; gT += sT; }
    });
...
function renderCharts(tableData, selThn, selPoli, allPoliNames) {
    ...
    let lineValues = listBulan.map(b => {
        let sum = 0;
        activePoli.forEach(p => targetTahun.forEach(t => { if(cachedData[b] && cachedData[b][p]) sum += cachedData[b][p][t].t; }));
        return sum;
    });
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, "Laporan_Kunjungan_Cilincing_2026.xlsx");
}
